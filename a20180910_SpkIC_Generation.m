...
dataLoc = sprintf('Data\\%s %s\\%s\\',experiment_name,IC_param,experiment_setup);
...
mask = calcSpkMask(spk_IC(:,:,5),fs,'alpha',0.01);
taxis = 0:1/fs:length(mixed)/fs-1/fs;
figure; imagesc(taxis,cf,mask); set(gca,'ydir','normal')
